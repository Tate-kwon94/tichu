#!/bin/zsh
# 3단 자가대전 RL 루프 — 학습팀 vs 동결 2단, 오라클 baseline + v6 KL앵커 + 트립와이어.
...
FROZEN=$ML/data/v6-weights.json          # 동결 2단 상대(=v6 정책 + keepSpecials 교환)
REFPT=$ML/data/v6-weights.pt             # KL 앵커 기준
KLTARGET=0.04                            # 적응형 KL 목표(β 자동 조정) — 세대당 이동 약간↑
[ -f "$RL/beta" ] && BETA=$(cat "$RL/beta") || BETA=0.4   # 세대 간 β 이월

# 초기화
if [ ! -f "$RL/cur.pt" ]; then
  cp "$ML/data/v6-weights.pt" "$RL/cur.pt"
  cp "$ML/data/v6-weights.json" "$RL/cur.json"
  echo "0" > "$RL/state"
  echo "$(date '+%H:%M:%S') init from v6" >> "$RL/loop.log"
fi
GEN=$(cat "$RL/state")
DROP=0
...
while [ "$GEN" -lt "$MAXGEN" ]; do
  ...
  for i in 0 1 2 3 4 5 6 7; do
    $NODE $ML/gen-rl.js "$RL/cur.json" "$FROZEN" $GPW $EPS $((GEN * 100000 + i * 10000)) "$DECL" "$ORACLE" \
