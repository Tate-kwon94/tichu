...
# 집계: python3 <merge-dan3.py> "ml/data/dan3x_*.log"
...
for lane in 0 1 2 3 4 5 6 7; do
  (
    for chunk in 0 1 2 3; do
      OUT="ml/data/dan3x_${lane}_${chunk}.log"
      # 이어하기: 완료된 청크(RESULT 줄 존재)는 건너뛴다
      if [ -f "$OUT" ] && grep -q "RESULT" "$OUT"; then continue; fi
      S=$((9001 + lane*100 + chunk*15))
      TICHU_CAND=exchange,learnExch TICHU_OPP_CAND=exchange \
...
wait
echo "완료 — 집계: merge-dan3.py 'ml/data/dan3x_*.log'"
